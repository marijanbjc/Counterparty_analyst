...
### Когда инструменты уходят в модель

Схемы 24 инструментов весят 1 817 токенов — больше четверти бюджета хода.
Поэтому прикладываются они не всегда.

| Ход | Инструменты у модели | Почему |
|---|---|---|
| Разбор, бесплатный тариф | нет | Код уже положил факт-пакет — запрашивать нечего |
| Уточнение, бесплатный тариф | нет | Со схемами модель тратила ход на выбор инструмента и отвечала фрагментом в 120 символов; без них на тех же данных ответ полный |
| Сравнение | нет | Сводка посчитана кодом |
| **Расширенный тариф** | **да, до 4 раундов** | Модель сама решает, чего не хватает |

**Что видит модель, когда инструменты приложены.** Не все 24, а девять из ядра:
базовая карточка, правовой статус, факторы риска, полный отчёт, сравнение,
углублённый разбор, график, вопросы к контрагенту и `load_tools`. Предметные
области — финансы, суды, владельцы, деятельность — по умолчанию скрыты.

**`load_tools(area)` — подгрузка области по требованию.** Если нужного
инструмента среди девяти нет, модель вызывает `load_tools("activity")`
и следующим шагом получает доступ к лицензиям, проверкам и госзакупкам.
Это экономит контекст: вместо 1 817 токенов на все области в промпт уходит
только ядро, а остальное появляется, когда действительно понадобилось.

Роутер при этом подстраховывает: если в сообщении есть слова «лицензия»,
«суд», «учредитель», нужная область открывается сразу, не тратя раунд
на `load_tools`.

**Ограничитель — белый список ИНН.** Диспетчер пропускает вызов только по тем
контрагентам, которые уже назывались в сессии или пришли из ответа предыдущего
инструмента. Модель не может уйти гулять по базе.
