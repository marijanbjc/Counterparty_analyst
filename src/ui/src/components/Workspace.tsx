...
import { date, money, number, riskName, riskTone } from '../format'
import type {
  AuthState,
  FactPack,
  Message,
...
function ServiceCard({
  ...
  return (
    <Button
      view="transparent"
      className={`service-card ${ai ? 'service-card-ai' : ''}`}
      onClick={onClick}
      disabled={!ai}
    >
      ...
      {ai && <span className="ai-card-action">Открыть <Icon name="chevron" size={14} /></span>}
    </Button>
  )
}
    ...
    <div className="contractors-page">
      ...
      <section className="services-grid" aria-label="Сервисы для контрагентов">
        {SERVICES.map((service) => (
          <ServiceCard
            key={service.title}
            {...service}
            onClick={service.ai ? onOpenAi : undefined}
          />
        ))}
      </section>

      <section className="contractor-search-card">
        <h2>Проверить контрагента</h2>
        <Input
          block
          size={48}
          placeholder="Поиск по названию компании, директору или ИНН"
          rightAddons={<Icon name="search" size={20} />}
      ...
      <section className="reports-section">
        <div className="reports-title">
          <h2>Полные отчёты</h2>
          <span>Результаты проверок в этом прототипе</span>
        </div>
        <div className="reports-table">
          <div className="reports-row reports-head">
            <span>Контрагент</span><span>Дата отчёта</span><span>Статус</span><span />
          </div>
          {currentPack ? (
            <div className="reports-row">
              <div><strong>{currentPack.short_name}</strong><small>ИНН {currentPack.inn} · ОГРН {currentPack.ogrn}</small></div>
              <span>{date(currentPack.as_of)}</span>
              <span className="report-status">Сформирован</span>
              <Button view="secondary" size={32} onClick={onOpenAi}>Открыть</Button>
            </div>
          ) : (
            <div className="reports-empty">
              <span className="empty-report-icon"><Icon name="statement" size={24} /></span>
              <div><strong>Отчётов пока нет</strong><p>Откройте «Проверку с ИИ» и отправьте ИНН.</p></div>
              <Button view="secondary" size={40} onClick={onOpenAi}>Начать проверку</Button>
            </div>
          )}
        </div>
      </section>
  ...
  return (
    <aside className="ai-dossier">
      <header>
        ...
        <p>ИНН {pack.inn} · ОГРН {pack.ogrn}</p>
...
function AiDialog({
  ...
  onSessionUpdated,
  onMessageAdded,
}: Omit<Props, 'profile' | 'onLogout'> & { onClose: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      onMessageAdded({
        ...
        meta: { inn: response.report.inn, report: response.report, degraded: true },
        created_at: new Date().toISOString(),
      }, response.report)
      setMessage('')
  ...
  return (
    <div className="ai-overlay" role="presentation" onMouseDown={(event) => {
      if (event.target === event.currentTarget) onClose()
    }}>
      <div className="ai-dialog" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="ai-dialog-title" tabIndex={-1}>
        <header className="ai-dialog-header">
          <div>
            <span className="ai-title-icon"><Icon name="spark" size={20} /></span>
            <div><h2 id="ai-dialog-title">Проверка с ИИ</h2><p>Анализ контрагента по данным отчёта</p></div>
          </div>
          <div className="ai-header-actions">
            <Button
              ...
              onClick={() => activeSession && void api.exportReport(auth.token, activeSession.id, 'md')}
            >
              Скачать
            </Button>
            <Button view="transparent" size={40} className="close-dialog" aria-label="Закрыть" onClick={onClose}>
        ...
        <div className="ai-dialog-body">
          ...
          <section className={`ai-chat ${mobileView !== 'chat' ? 'mobile-panel-hidden' : ''}`}>
            <div className="ai-role-switch">
              <span>Фокус анализа</span>
              <div>
                {ROLES.map((item) => (
                  <Button
                    key={item.value}
            ...
            <form className="ai-composer" onSubmit={submit}>
              ...
              <small>Пока отвечает детерминированный backend. Агент будет подключён позже.</small>
            </form>
          </section>
...
export function Workspace(props: Props) {
  const {
    ...
    onLogout, onCreateSession, onSelectSession, onSessionUpdated, onMessageAdded,
  } = props
  ...
  const openAi = () => {
    aiTriggerRef.current = document.activeElement as HTMLElement | null
    setAiOpen(true)
  }
  const closeAi = () => {
    setAiOpen(false)
  ...
  useEffect(() => {
    if (aiOpen) shellRef.current?.setAttribute('inert', '')
    else shellRef.current?.removeAttribute('inert')
  }, [aiOpen])
  ...
  return (
    <div className="business-app">
      <div className="business-shell" ref={shellRef}>
        <BusinessSidebar />
        <main className="business-main">
          ...
          <ContractorsPage currentPack={currentPack} onOpenAi={openAi} />
        </main>
