...
# Подписи уходят в текст плашки и должны совпадать с кнопками в интерфейсе:
# клиент не поймёт, что «Юридический» — это кнопка «Суды и взыскания».
LABELS: dict[str, str] = {
    FINANCE: "Финансы",
    LEGAL: "Суды и взыскания",
    SECURITY: "Статус и владельцы",
    ACTIVITY: "Чем занимается",
    FOLLOWUPS: "Что запросить у контрагента",
}
