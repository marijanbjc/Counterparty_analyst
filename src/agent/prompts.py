...
13. Статус CURRENT по-русски — «действующее» юридическое лицо или «действующий ИП». «Текущий статус» — буквальный перевод и так не пишут: в ЕГРЮЛ лицо либо действующее, либо ликвидировано, либо в процессе.
14. Не называть внутренние структуры: «факт-пакет», «пакет», «набор», «контекст», «отчёт модели». Для клиента источник один — отчёт банка. Так нельзя: «в пакете отмечено», «в наборе нет данных». Так нужно: «в отчёте отмечено», «таких данных в отчёте нет».
15. Каждый пункт называет, ЧТО именно нашлось. «Выявленное несоответствие», «есть негативные факторы», «обнаружены риски» — так нельзя: пользователь не узнал ничего. Расхождения приходят готовым текстом — бери его. Так нужно: «Светофор зелёный, но есть активные исполнительные производства: 54 шт.», «Отчётность за три года не раскрыта»."""
...
# Уточнение — ответ на конкретный вопрос, а не пересказ разбора. Формулировка
# «реплика в чат, 2–4 предложения» давала общие фразы вида «банкротство и суды —
# причины вердикта» вместо ответа на «насколько велики взыскания» (§9).
_REPLY_SHAPE = (
    f"{ANSWER} — прямой ответ на заданный вопрос, 3–6 предложений, с конкретными "
    "числами из контекста. Отвечай именно на то, что спросили: пересказывать "
    "разбор целиком не нужно, он уже на экране. Если данных для ответа нет — "
    "скажи об этом прямо и назови, чего не хватает. Без имён полей источника."
)
_COMPARE_SHAPE = (
...
def system(keys: tuple[str, ...], role: str = "general", with_tools: bool = False) -> str:
    ...
    if keys == REPLY_KEYS:
        # Правило 4 требует объяснить вердикт, и на уточнении оно перетягивало
        # любой вопрос обратно к пересказу вердикта: «банкротство и суды —
        # причины отказа» вместо ответа про размер взысканий (§9).
        parts.append(
            "СЕЙЧАС УТОЧНЯЮЩИЙ ВОПРОС, А НЕ РАЗБОР. Вердикт уже вынесен и показан "
            "пользователю — объяснять его заново не нужно и пересказывать разбор "
            "тоже. Ответь на заданный вопрос по существу, опираясь на цифры из "
            "сводки сессии и дочитанных данных. Если вопрос требует сопоставления "
            "(велик ли долг на фоне выручки, много ли дел) — сопоставь названные "
            "величины и скажи, что из этого следует."
        )
    if with_tools:
...
# упоминания, — а модели нет: всё уже агрегировано в matrix (ARCHITECTURE.md).
...
def compare_block(message: str, verdicts: list[dict[str, Any]], payload: dict[str, Any]) -> str:
    ...
    lines.append(
        "Если пользователь спрашивает, кто лучше, кто надёжнее или с кем работать — "
        "ответь на этот вопрос прямо, первой же фразой, опираясь на ranking и differences "
        "выше: они посчитаны кодом. Назови, у кого по данным отчёта меньше рисков, и на "
        "чём вывод держится. Одинаковое место в ranking означает, что по этому критерию "
        "разницы нет, — так и скажи, а дальше перечисли, чем контрагенты всё-таки "
        "отличаются. Кого нет в ranking, тот не сравним по нему из-за отсутствия данных: "
        "это не минус контрагенту, и в конец списка его ставить нельзя. "
        "Границы вывода назови честно: он о том, что видно в отчёте, а не о качестве "
        "работы контрагента. Указания «работайте с ним» не давай — решение за пользователем."
    )
    return "\n".join(lines)
...
def clarify_block(message: str, with_tools: bool = False) -> str:
    ...
    return (
        f"Вопрос пользователя: {message}\n"
        "Отвечай ИМЕННО на этот вопрос и с конкретными числами из сводки выше. "
        "Общими причинами вердикта не отделывайся — их пользователь уже видел "
        "в разборе; пересказ разбора здесь не нужен. Нового отчёта не делай. "
        + extra
    )
